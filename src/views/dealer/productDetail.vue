    ...
    <div v-if="domShow">
      ...
      <div class="info">
        ...
        <div class="right-box">
          ...
          <div class="p-price-column">
            ...
            <div v-else>
              ...
              <div class="p-p-wrap" v-else>
                ...
                <div class="p-p-c-r">
                  <div class="price" v-if="productInfo.saleMode!=6">
                  ...
                  <!--经销商产品-->
                  <div v-else class="price" v-html="$options.filters.dealerPrice(productInfo)"></div>
                  <div class="tip" v-if="productInfo.hiddenPriceText">{{productInfo.hiddenPriceText}}</div>
                  <!--显示划掉的价格-->
                  <div class="elide-price">
                    <!--凑单-->
          ...
          <!--经销商领券-->
          <div class="p-coupon-receive" v-if="productInfo.shopCouponTag&&productInfo.shopCouponTag.tagItemDetails.length">
            <span>领券</span>
            <ul v-for="item in productInfo.shopCouponTag.tagItemDetails">
              <li>{{item.tagDesc}}</li>
            </ul>
            <div class="receive-btn" @click="unwindCoupon">
              <i :class="{'flip':dealerCouponShow}"></i>
            </div>
            <!--领券面板-->
            <coupon-panel v-if="dealerCouponShow" class="coupon-panel" :coupon-list="dealerCouponList"></coupon-panel>
          </div>
          <!--商品提示栏-->
                ...
                <div v-if="userState==1">
                  ...
                  <div v-if="productInfo.productState!=0">
                    ...
                    <div v-if="bottomBarState==8">
                      <span class="add" @click="_dealerApply">申请进货</span>
                    </div>
                    <!--会员等级不够-->
  ...
  export default {
    data() {
      return {
        ...
        toProductParam: '',
        shopId: ``,//经销商店铺id
        dealerCouponList: [],//经销商优惠券
        dealerCouponShow: false,
    ...
    methods: {
        ...
        productApi.GetProductDetail(param,type)
          .then(res => {
            if (res.success) {
              ...
              Object.assign(productDetail,productDetail.productPrice)
      ...
      //展开经销商优惠券领取面板
      unwindCoupon(){
        if(this.dealerCouponShow){
          this.dealerCouponShow = false
          return
        }
        if(this.dealerCouponList.length){
          this.dealerCouponShow = true
          return
        }
        let param = {
          data: this.shopId,
        ...
        queryDealerCoupons(param)
          .then(res => {
            if (res.success) {
              this.dealerCouponList = res.data
              this.dealerCouponShow = true
            }
          })
          .catch(res => {});
      ...
      _dealerApply(){
        this.productInfo.imgUrl = this.productInfo.imgUrl || this.productInfo.imgsUrl[0]
        bus.$emit("popShow",this.productInfo);
      },
    },
  }
</script>
